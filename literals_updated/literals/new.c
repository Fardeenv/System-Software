        ...
        if (strcmp(opcode, "END") != 0)
        {
            ...
            if (operand[0] == '=')
            {
                ...
                if (operand[1] == 'C') {
                    ...
                    LITTAB[lit_count++].addr = 0; // Address 0 is placeholder until it's processed later
                } else if (operand[1] == 'X') {
    ...
    struct
{
    ...
    for (int i = 0; i < read_count; i++) {
        ...
        if (search_optab(inter[i].opcode, &instr_length)) {
            ...
            int lit_found = 0;
            // Search for the literal in LITTAB
            for (int k = 0; k < lit_count; k++) {
                if (!strcmp(LITTAB[k].constant, inter[i].operand)) {
                    lit_found = 1;
                    break;
                }
            }
            ...
            //printf("Checking literal operand: '%s'\n", inter[i].operand);

            if (inter[i].operand[0] == '=') {
            printf("Checking literal operand: %s\n", inter[i].operand); // Debugging line
            // Handle literal operand (look in LITTAB)
            int lit_index = -1;
            for (int j = 0; j < lit_count; j++) {
                if (strcmp(LITTAB[j].constant, inter[i].operand) == 0) {
                    lit_index = j;
                    break;
                }
            }
            ...
            if (lit_index != -1) {
                snprintf(obj_code, sizeof(obj_code), "%06X", LITTAB[lit_index].addr);
                printf("Literal found at address: %X\n", LITTAB[lit_index].addr); // Debugging line
            } else {
