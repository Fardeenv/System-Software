...
int search_sym(
    const char* name) {  // returns 2 if symbol is not present in symbol table
...
int is_sym_def(const char* name) {
  ...
  if (k == -2)
    return 0;  // check the return here???
  else
    return sym[k].def;
}
...
void add_sym_undef(char* symname, int address, const char* codeline) {
  ...
  if (k == -2) {
    ...
    po->next = NULL;
    t->next = po;
    //	printf("Added addres to Symbol %s with adddres
    //%06X\n",symname,po->address);
  }
}
...
void check_undefined_symbols() {
  for (int i = 0; i < symcount; i++) {
    if (!sym[i].def) {
      printf("\nERROR: Undefined symbol '%s' in the code.\n", sym[i].name);
    }
  }
}
...
void add_sym_def(char* symname, int value, FILE* fout) {
  ...
  sym_addr *po, *t;
  char label[100], opr1[100], opr2[10], opc[100], opr[200], li[300], count = 0,
                                                                     ind = 0;
  if (k == -2) {
    strcpy(sym[symcount].name, symname);
    ...
    if (is_sym_def(symname)) {
      printf("\nERROR: Duplicate symbol '%s' detected.\n", symname);
...
void one_pass(const char* input, const char* output) {
  ...
  int end = 0;
  int count = start, flag = 0, temp = 0, cmnt = 0, fl = 0, jj = 0, wordb = 0,
      opcode = 0, operand = 0, ind = 0;
  char label[100], opc[100], opr[100], li[200], startadd[10], opr1[50],
      opr2[10], byt[8], pname[10];
...
int write_textrec(FILE* fil, int addr, int bytesize, int word,
                  int new) {       // when new==0 default
  ...
  char word_s[15], *ch;
  if (bytesize == 3)
    sprintf(word_s, "%06X", word);
  else if (bytesize == 2)
    sprintf(word_s, "%04X", word);
  else if (bytesize == 1)
    sprintf(word_s, "%02X", word);
  if (new == 0) {
    if (tsize > 27) {
