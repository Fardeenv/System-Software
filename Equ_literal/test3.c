...
void pass1()
{
    ...
    FILE* f = fopen("instructions_EQU.txt", "r");
    ...
    while (fgets(line, sizeof(line), f))
    {
        ...
        while (expression[z] != '\0') {
            // Skip spaces
            while (isspace(expression[z])) z++;

            // Parse the next token (either a number or a symbol)
            int j = 0;
            while (expression[z] != '\0' && !strchr("+-", expression[z]) && !isspace(expression[z])) {
                token[j++] = expression[z++];
            }
            token[j] = '\0';
                ...
                // Check if token is a symbol or a number
                ...
                if (strchr("+-", expression[z])) {
                    operator = expression[z++];
                } else {
                    operator = '+';
                }
            }
            ...
            // Write the resolved value instead of the original expression
            fprintf(output, "%lX %s %s %lX\n", pc, label, opcode, equ_value);
            continue;
        }
